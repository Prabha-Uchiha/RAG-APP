...
Question: {question}""",
    ...
    def answer_with_rag(self, question: str, num_retrieved_docs: int = 5) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            context += "".join([
                f"Document {str(i)}:::\n{doc}\n\n" 
                for i, doc in enumerate(relevant_docs_text)
            ])
...
Please provide a comprehensive answer based on the context provided above. If the answer cannot be found in the context, please say so."""
            
            # Generate answer
            if self.reader_llm:
                response = self.reader_llm(final_prompt, max_length=200, do_sample=True, temperature=0.7)
...
if __name__ == "__main__":
    # Install required packages notification
    st.markdown("""
    ### Required Dependencies:
    ```bash
    pip install streamlit torch transformers langchain langchain-community 
    pip install sentence-transformers faiss-cpu PyMuPDF python-docx
